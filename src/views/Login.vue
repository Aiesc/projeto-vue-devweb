<template>
  <v-container fill-height fluid text-center>
    <v-container>
      <v-row class="elavation-3 mxauto justify-center">
        <v-col cols="auto">
          <v-img src="@/assets/images/logo.png">
          </v-img>
          <v-form>
            <v-text-field label="Email" v-model="user.email"></v-text-field>
            <v-text-field label="Senha" v-model="user.password" :type="show? 'text': 'password'" :append-icon="show ? 'mdi-eye': 'mdi-eye-off'" @click:append="show = !show" @keydown.enter="login"></v-text-field>
